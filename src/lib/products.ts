...
import { PRODUCTS as CATALOG, type AvailableProduct, type Product } from '../../shared/products';
...
export const PRODUCTS: readonly Product[] = CATALOG.map((product) => ({
  ...
  ...(product.availability === 'available' ? { pricingUrl: new URL(new URL(product.pricingUrl).pathname, SITE_ORIGIN).href } : {}),
}));
